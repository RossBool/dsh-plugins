...
| `dsh-team-mode` | 团队模式开关：为协作画布、协作路由等团队协作能力提供进程级与会话级启停 |
...
| `dsh-mcp-manager-stub` | 占位包：接管旧客户端条目 id，保持 boot 清单干净（无操作） |
...
部分带客户端 UI 的插件（`dsh-agent-orchestration`、`dsh-team-mode`、`dsh-voice` 等）的浏览器端 bundle `dist/client.js` 为构建产物，未纳入版本控制。使用时先安装依赖并构建：
